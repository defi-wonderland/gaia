...
/// Stream action logs from Amp via Arrow Flight SQL.
///
/// Connects to the Amp server, executes a streaming SQL query filtered by the
/// Space Registry contract address, groups rows by block, sorts actions by
/// `log_index` for deterministic ordering, and calls `handle_block` for each
/// complete block. Automatically reconnects on stream end or error.
    ...
    Fut: Future<Output = Result<()>> + Send,
{
    ...
    loop {
        ...
        match stream_inner(&config.flight_url, &sql, resume_from, &mut handle_block).await {
            Ok(last_block) => {
                resume_from = last_block.map_or(resume_from, |b| b.saturating_add(1));
                warn!(
                    resume_from,
            ...
            Err(e) => {
                warn!(
                    resume_from,
                    error = %e,
                    "Amp Flight stream error, reconnecting after delay"
                );
            }
        }
...
/// Inner streaming loop. Returns the last processed block number (if any).
async fn stream_inner<F, Fut>(
    flight_url: &str,
    sql: &str,
    _start_block: u64,
    handle_block: &mut F,
...
where
    ...
    Fut: Future<Output = Result<()>> + Send,
{
    let flight_client = FlightServiceClient::connect(flight_url.to_string())
        .await
    ...
    while let Some(batch) = stream.next().await {
        ...
        for row in 0..batch.num_rows() {
            ...
            // Block boundary: flush the previous block
            if current_block != Some(block_num) {
                if let Some(prev_block) = current_block {
                    ...
                    handle_block(block).await?;
                    last_processed_block = Some(prev_block);
                }
